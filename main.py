...
cube, rect_height, rect_width = 20, 20, 10  #размер одной любой фигуры(20х20 пикселей), ширина и высота игрового поля(прибл. одинаковые чтобы можно было привязать к фигуре по размерам)
...
def clear_field(rectangle):  #очистка поля
    ...
    while y >= 0:  #если координата ряда поля больше либо равна 0
        if full_filled_rows(rectangle, y):  #функция по проверке заполненных рядов
            ...
            removed_rows += 1 #если функция вернула тру(очистила ряд в поле), прибавляем 1 ряд к счетчику удаленных рядов
        else:  #иначе(если функция вернула фолс)
            y -= 1  #ничего не происходит (если заполненных рядов пока нет)
...
def get_figure():  #чтобы получать случайную фигуру по цвету и случайный угол поворота фигуры
    ...
    new_figure = {"shape": shape,
                  ...
                  "x": int(rect_width / 2) - int(rect_width / 2),  #получаем координаты х учитывая ширину поля
...
def draw_info_labels(points, level): #прорисовувываем остальные надписи, которые говорят о текущем уровне и количестве очков
    ...
    points_rect.topleft = (app_width - 550, 180)
    ...
    level_rect.topleft = (app_width - 550, 250)
    ...
    escape_rect.topleft = (app_width - 550, 420)
    escape_rect2.topleft = (app_width - 550, 440)
...
def figure_speed(points):  #скорость падения фигуры зависит от уровня
    level = int(points / 4) + 1  #увеличивать уровень на один при делении полученного кол-ва очков на кол-во очков выделенное на один уровень(4)
    fall_figure_speed = 0.27 - (level * 0.02)  #скорость падения фигуры уменьшается
...
#интерпретатор будет искать импортированый файл и перед выполнением этого модуля он присвоит имя из оператора import переменной __name__